...
public class Structure 
{
	...
	// Constructuer d'une structure à partir d'un nombre de cuve
	...
	// Constructeur d'une structure à partir d'un fichier préfait
	...
	// Méthodes lié à la manipulation des attributs

	// Permet de créer une cuve dans le tableau de cuve à l'indice renseigné
    public void creerCuve(int indice, int quantite) 
	{
		this.tabCuve[indice] = Cuve.fabriquerCuve(quantite);
	}

	// Permet de relier deux cuves à partir des deux identifiant renseigné en prenant en compte
	// une taille de section
	public boolean ajouterRelation(char c1, char c2, int section)
    {
        // Vérification que les valeurs font parti des cuves existants
        if( c1 < 'A' || c1 >= (char)('A'+this.tabCuve.length) || 
		    c2 < 'A' || c2 >= (char)('A'+this.tabCuve.length)   ) return false;

		// Vérification que la valeur de la section est correcte
		if( section < 2 || section > 10 )                         return false;

		// Vérification que la relation n'existe pas déja
		if( this.tabRelation[(int)c1 -'A'][(int)c2 -'A'] != 0 )   return false;

		// Vérification que les cuves ne sont pas identiques
	...
	// Permet d'optimiser les positions des cuves en formant un cercle
	...
	public int    getRelation(int c1, int c2) { return this.tabRelation[c1][c2];           }
	public char   getIdCuve  (int indice)     { return this.tabCuve[indice].getIdCuve  (); }
	public double getContenu (int indice)     { return this.tabCuve[indice].getContenu (); }
	public int    getCapacite(int indice)     { return this.tabCuve[indice].getCapacite(); }
	public int    getPosX    (int indice)     { return this.tabCuve[indice].getPosX    (); }
	public int    getPosY    (int indice)     { return this.tabCuve[indice].getPosY    (); }
	public String getPosInfos(int indice)     { return this.tabCuve[indice].getPosInfos(); }
	...
	// Méthodes lié à la création/manipulation de fichier

	// Méthode permettant de générer le fichier .txt qui acceuillera les informations de la Structure
    public boolean genererTxt(String typeStructure, String nomFichier)
    {
        ...
        for (String t : typeRelation)
			//L'argument renseigné par l'utilisateur doit être une String présente dans typeRelation
			//sinon, la méthode retourne false
            if (typeStructure.equals(t))
            {
                try
                {
					//Si aucun nom de fichier n'est donné, il est nommé Structure.txt
					...
					for (int cpt1 = 0; cpt1 < this.tabCuve.length; cpt1++)
					{
						...
						//Si l'utilisateur tape "lstAdj", une liste d'adjacence sera créé dans le .txt
						...
						//Si l'utilisateur tape "matCout", une matrice de coût sera créé dans le .txt
						else if (typeStructure.equals("matCout"))
						{
							for (int cpt2 = 0 ; cpt2 < this.tabCuve.length ; cpt2++) 
								pw.print(this.tabRelation[cpt1][cpt2] + "\t");
						}
						//Si l'utilisateur tape autre chose(obligatoirement "matCoutOpt"), une
						//matrice de coût optimisée sera créé dans le .txt
						else
						{
	...
	// Méthodes lié aux échange de contenu entre les cuves
			...
			for (int cpt1 = cpt; cpt1 < this.tabCuve.length; cpt1++)
				if (this.tabRelation[cpt][cpt1] != 0) 
                {
					if ( (this.tabCuve[cpt].getContenu() + liquideModifier[cpt]) > (this.tabCuve[cpt1].getContenu() + liquideModifier[cpt1]) )
						this.calculeQuantiteTransfertLiquide(cpt, cpt1, liquideModifier, this.tabRelation[cpt][cpt1]);
					else if ( (this.tabCuve[cpt].getContenu() + liquideModifier[cpt]) < (this.tabCuve[cpt1].getContenu() + liquideModifier[cpt1]) )
						this.calculeQuantiteTransfertLiquide(cpt1, cpt, liquideModifier, this.tabRelation[cpt][cpt1]);
				}
	...
	private void calculeQuantiteTransfertLiquide( int indexCuve1, int indexCuve2, double[] liquideModifier, int relation ) 
    {
