...
public class PanelLiaisons extends JPanel implements ActionListener 
{
    ...
    public PanelLiaisons(Controleur ctrl, FrameNouveau frame) 
	{
        this.ctrl  = ctrl;
        this.frame = frame;
    ...
    public void actionPerformed(ActionEvent e) 
	{
        ...
        if (e.getSource() == this.btnLstAdj || e.getSource() == this.btnMatCout || 
		    e.getSource() == this.btnMatCoutOpt                                    )
		{

			//si le texte est vide, affciher le message
			if (this.txtNom.getText().equals(""))
			{
				this.frame.afficherPopUp("Veuillez renseigner le nom du fichier");
			}
			else //sinon créer le fichier
			{
				if      (e.getSource() == this.btnLstAdj) 
					ctrl.genererTxt("lstAdj", this.txtNom.getText());
				else if (e.getSource() == this.btnMatCout) 
					ctrl.genererTxt("matCout", this.txtNom.getText());
				else
					ctrl.genererTxt("matCoutOpt", this.txtNom.getText());

				this.ctrl.ouvrir(new File(this.txtNom.getText() + ".txt"));
				this.frame.dispose();;
			}
		}
